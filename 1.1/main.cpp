...
int main()
{
	...
	//Datenlücken (999er) interpolieren
	{
		...
		if(sits[0].ret_cl() == 999 || sits[0].ret_cl() == -999 || sits[0].ret_cl() == -1)
		{
		...
		{
			if(sits[i].ret_cl() == 999 || sits[i].ret_cl() == -999 || sits[i].ret_cl() == -1)
			{
				i_start = i-1;
				cl_start = sits[i-1].ret_cl();
				while(sits[i].ret_cl() == 999 || sits[i].ret_cl() == -999 || sits[i].ret_cl() == -1)
				{
					i++;
				}
				i_end = i;
